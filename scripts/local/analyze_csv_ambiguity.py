...
COLORS_PATH = os.path.join(BASE_DIR, "colors.csv.gz") # Note: listed as .csv.gz in previous `list_dir`
...
def analyze_ambiguity():
    ...
    grouped = elements_df.groupby(['part_num', 'color_id'])['element_id'].nunique().reset_index()
    grouped.columns = ['part_num', 'color_id', 'variation_count']
    
    # Filter for cases with > 1 variation
    ambiguous_parts = grouped[grouped['variation_count'] > 1].sort_values('variation_count', ascending=False)
    ...
    summary.append(f"Percentage of ambiguity: {len(ambiguous_parts)/len(grouped)*100:.2f}%")
    ...
    for index, row in ambiguous_parts.head(20).iterrows():
        ...
        # Get names
        part_name = parts_df[parts_df['part_num'] == part_num]['name'].values[0] if not parts_df[parts_df['part_num'] == part_num].empty else "Unknown"
        color_name = colors_df[colors_df['id'] == color_id]['name'].values[0] if not colors_df[colors_df['id'] == color_id].empty else f"Color {color_id}"
        
        # Get the list of Element IDs
        ids = elements_df[(elements_df['part_num'] == part_num) & (elements_df['color_id'] == color_id)]['element_id'].tolist()
        
        
        report_lines = []
        report_lines.append(f"Part: {part_num} ({part_name}) | Color: {color_name} | Variations: {count}")
