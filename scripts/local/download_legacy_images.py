        ...
        for future in as_completed(futures):
            ...
            if processed % 100 == 0:
                ...
                params = (
                    processed,
                    total_images,
                    processed / total_images * 100,
                    rate,
                    stats["downloaded"],
                    stats["exists"],
                )
                sys.stdout.write(
                    f"\rProgress: {processed}/{total_images} ({processed/total_images*100:.1f}%) | Rate: {rate:.1f} img/s | New: {stats['downloaded']} | Exists: {stats['exists']}"
                )
                sys.stdout.flush()
